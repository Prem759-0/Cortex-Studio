...
export const CanvasArea: React.FC = () => {
  ...
  const getCoordinates = (e: React.PointerEvent<HTMLCanvasElement>): Point | null => {
    ...
    // Calculate exact coordinates based on scale and translation
    return { 
    ...
    [...layers].reverse().forEach(layer => {
      ...
      strokesToRender.forEach(stroke => {
        ...
        if (stroke.tool === 'eraser') {
          ...
          offCtx.strokeStyle = stroke.color + '80'; // 50% opacity hex
  ...
  useEffect(() => {
    const resizeCanvas = () => {
      if (containerRef.current && canvasRef.current) {
        // Keep actual canvas resolution static for performance, scale via CSS
  ...
  return (
    <div 
      ref={containerRef} 
      onWheel={handleWheel}
      className="w-full h-full relative overflow-hidden bg-zinc-100 dark:bg-zinc-950/80 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0MCIgaGVpZ2h0PSI0MCI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9IiNlMmU4ZjAiLz48L3N2Zz4=')]"
    >
      <div 
        style={{ 
        ...
        className={`absolute top-1/2 left-1/2 -mt-[540px] -ml-[960px] w-[1920px] h-[1080px] bg-white shadow-2xl transition-transform ${activeTool === 'pan' || isPanning ? 'cursor-grab active:cursor-grabbing' : 'cursor-crosshair'}`}
      >
        <motion.canvas
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
